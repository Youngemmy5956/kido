                    ...
                    <form action="{{ route('storeUser') }}" method="post" class="space-y-6">
                        ...
                        <div class="flex" class="w-full">
                            <input type="text" value="{{ old('first_name') }}" name="first_name"
                                class="block border h-12 p-3 w-6/12 mr-2" placeholder="First Name">
                            <input type="text" value="{{ old('last_name') }}" name="last_name"
                                class="block border h-12 p-3 w-6/12 ml-2" placeholder="Last Name">
                        </div>
                        @error('email')
                            <p class="text-red-500">{{ $message }}</p>
                        @enderror
                        <input type="email" name="email" value="{{ old('email') }}"
                            class="block border w-full h-12 p-3 focus:border-green-500 focus:border-2"
                            placeholder="Email">
                        @error('password')
                            <p class="text-red-500">{{ $message }}</p>
                        @enderror
                        ...
                        <input type="submit" value="Sign in"
                            class="bg-green-500 text-white w-full py-4 hover:bg-white hover:text-green-500 hover:border-2 hover:border-green-500">

                    </form>
                    <div class="text-center"><a href="{{ route('login') }}" class="text-green-500">Already have an
                            account?</a></div>
                </div>
